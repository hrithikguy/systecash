...
;;   - a list of all the known systecash accounts in
;;     (shortname fullname Account*) format
;;   - a hash of QIF account name to systecash account info
;;   - a hash of QIF category to systecash account info
;;   - a hash of QIF memo/payee to systecash account info
...
(define (qif-import:load-map-prefs)
  ...
  ;; systecash account. Each member of the list is itself a list with
  ...
  (let* ((pref-filename (gnc-build-dotsystecash-path "qif-accounts-map"))
         (results '()))
...
;;  with existing systecash commodities.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    (for-each
     (lambda (entry)
       (if (and (list? entry)
           ...
           ;; list of three items: the QIF name, and the systecash
           ...
           (let ((commodity (gnc-commodity-table-lookup
             ...
             (if (and commodity (not (null? commodity)))
                 ;; There is an existing systecash commodity for this
...
(define (qif-import:save-map-prefs acct-map cat-map memo-map
  ...
  (define (private-save)
    (with-output-to-file (gnc-build-dotsystecash-path "qif-accounts-map")
      (lambda ()
        (display ";;; qif-accounts-map")
        (newline)
        (display ";;; Automatically generated by systecash. DO NOT EDIT.")
        (newline)
        (display ";;; (Unless you really, really want to.)")
        (newline)
        (display ";;; Map QIF accounts to systecash accounts")
        (newline)
        (qif-import:write-map acct-map)
        (newline)

        (display ";;; Map QIF categories to systecash accounts")
        (newline)
        (qif-import:write-map cat-map)
        (newline)

        (display ";;; Map QIF payee/memo to systecash accounts")
        (newline)
        (qif-import:write-map memo-map)
        (newline)

        (display ";;; Map QIF security names to systecash commodities")
        (newline)
        (qif-import:write-securities security-map security-prefs)
        (newline)

        (display ";;; systecash separator used in these mappings")
        (newline)
...
(define (qif-import:guess-acct acct-name allowed-types gnc-acct-info)
  ...
  ;; existing systecash account with a name that could reasonably
