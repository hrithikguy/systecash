 ...
 *            schema file org.systecash.warnings.gschema.xml.in       *
 *                                                                  *
...
<xsl:for-each select="//schema[@id='org.systecash.warnings.permanent']/key">
#define GNC_PREF_WARN_<xsl:value-of select="translate(@name,$smallcase,$uppercase)"/> "<xsl:value-of select="@name"/>"</xsl:for-each>

enum {<xsl:for-each select="//schema[@id='org.systecash.warnings.permanent']/key">
