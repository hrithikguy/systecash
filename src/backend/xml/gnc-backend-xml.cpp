...
/* Write the financial data in a book to a file, returning FALSE on
   ...
   "file.YYYYMMDDHHMMSS.systecash" where YYYYMMDDHHMMSS is replaced with the
...
gnc_xml_be_remove_old_files (FileBackend* be)
{
    ...
    while ((dent = g_dir_read_name (dir)) != NULL)
    {
        ...
        /* Ensure we only evaluate systecash related files. */
         ...
         * <fullpath/to/datafile><anything>.systecash
         * <fullpath/to/datafile><anything>.xac
         * <fullpath/to/datafile><anything>.log
         *
         * To be a file generated by systecash, the <anything> part should consist
         ...
         */
        {
            ...
            if (!got_date_stamp) /* Not a systecash created file after all... */
            {
                g_free (name);
                continue;
            }
...
{
    const char* name {"systecash File Backend Version 2"};
